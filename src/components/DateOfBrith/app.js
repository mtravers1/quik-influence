...
function datepickerUI(app) {
  return h(
    'div',
    {
      class: `dob-datepicker__wrapper ${
        app.state.show_datepicker ? '' : 'dob-datepicker__wrapper--hide'
      } `,
    },
    [
      h('div', {
        class: `dob-datepicker__background ${
          app.options.display_mode === 'popup'
      ...
      h(
        'div',
        {
          ...
          class: `dob-datepicker ${
            app.options.display_mode === 'popup' ? 'dob-datepicker--popup' : ''
          }`,
        ...
        [
          ...
          h(
            'div',
            { class: 'dob-datepicker__options' }, //wrapper for options
...
function topArrow(app) {
  return h('div', {
    class: `top-arrow ${
      app.options.display_mode === 'popup' ? 'top-arrow--popup' : ''
    }`,
  });
}
...
function monthSelection(app) {
  return h(
    ...
    app.options[app.options.show_long_month ? 'long_month' : 'short_month'].map(
      (month, index) => {
        return h(
          'button',
          {
    ...
    [...Array(app.state.dob.month === 2 ? 30 : 32).keys()].map((day, index) => {
      ...
      return day === 0
        ...
        : h(
            ...
            [
              text(day),
              app.options.enable_ordinal_number
                ? h(
                    'sup',
                    { class: 'dob-datepicker__day__ordinal-number' },
                    getOrdinalSufix(index)
                  )
                : null,
            ]
          );
...
function yearSection(app, children, earliest_year, oldest_year) {
  return h('div', { class: 'dob-datepicker__year__year-section' }, [
    h(
      ...
      text(
        `${app.options.labels.date_range_label}${oldest_year} - ${earliest_year}`
      )
    ),
        ...
        h(
          'button',
          {
            ...
            onclick: e => {
              ...
              app.renderDatepicker().hideDatepicker();
            },
          },
          [
            h(
              ...
              text(i.toString().substring(0, 2))
            ),
            h(
              'span',
              { class: 'dob-datepicker__year__button__last' },
              text(i.toString().substring(2))
            ),
          ]
        )
      );
      ...
      if (year_section.length === 20) {
        ...
        all_years.push(
          yearSection(
            ...
            year_section,
            year_range[0],
            year_range[year_range.length - 1]
          )
        );
