...
function getDefaultDateFormat(app) {
  return `${app.state.dob.year}-${pad(app.state.dob.month)}-${pad(
    app.state.dob.day
  )}`;
}
...
function assignDateToTargetNode(app) {
  ...
  const selected_date = app.options.dateFormat
    ? app.options.dateFormat(getDefaultDateFormat(app))
    : getDefaultDateFormat(app);
...
function storeValue(type, value, app) {
  ...
  if (app.state.dob.day && app.state.dob.month && app.state.dob.year) {
    app.options?.onChange(app.state.dob);
  }
