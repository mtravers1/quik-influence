...
const SearchPage = () => {
  ...
  const handleRemoveQuery = (id: number) => {
    ...
    setSearchParams((params: any) =>
      params.filter((param: any) => param.id !== id)
    );
  };
  ...
  const applySearch = () => {
    ...
    searchParams.forEach((searchParam: any) => {
      ...
      if (property && values) {
        ...
        if (
          Object.keys(filters[label]).length &&
          comparator in filters[label]
        ) {
          ...
          filters[label][comparator] = Array.isArray(values)
            ? values
            : [values];
        }
      }
  ...
  return (
    <Box>
      ...
      <Flex w="80%" p={6} marginTop={10} bg={basicTheme[colorMode]} wrap="wrap">
      ...
      {showResults && <SearchResults leads={allLeads} loading={loading} />}
    </Box>
  );
};
